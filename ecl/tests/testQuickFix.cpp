...
int main( int argc, char** argv )
{
  ...
  bpo::options_description cmdlineOptions( "Generic options" ) ;
  cmdlineOptions.add_options()
    ("port,p", bpo::value<short> ( &port ), "Port to bind")
    ("count,c", bpo::value<int> ( &count ), "Number of orders to test")
    ;

  bpo::variables_map vm ;
  store( bpo::command_line_parser( argc, argv).options( cmdlineOptions ).run(), 
         vm ) ;
  notify( vm ) ;
  ...
  else if ( !( vm.count ("count") || vm.count ("c") ) )
  {
    std::cout << cmdlineOptions << std::endl ;
    return 1; 
  }
  ...
  std::cout << "Sending/Receiving NewOrderSingle/ExecutionReports "
               "on ThreadedSocket";
